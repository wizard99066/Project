...
const ListBooks = () => {
	...
	const { changed: changedFavorite, isSending: isSendingFavorite, paged } = useSelector((state) => state.userBookFavoriteReducer)
	...
	const getPages = (values) => {
		dispatch(userBookFavoriteActions.getPaged(values))
	}
